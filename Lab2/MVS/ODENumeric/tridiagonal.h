...
std::vector<double> solve(size_t n, double a, double b, double m1, double m2, double xi, func k1, func k2, func q1, func q2, func f1, func f2) {
	...
	std::vector<double> ai(n+1), di(n), pi(n);            // массивы коэффициентов для построения системы

	// ВЫЧИСЛЕНИЕ КОЭФФИЦИЕНТОВ СЛАУ
	...
	x = a + h/2; // текущая точка вида 'x + h*(i + 1/2)'
	...
	*/
	...
	double hi = 1/h;                      // обратный шаг (множитель в выражениях ниже)
	std::vector<double> A(n), B(n), C(n); // коэффициенты на диагоналях матрицы системы
	...
	std::vector<double> y(n+1);                // вектор решения

	
	alpha[1] = 0, beta[1] = m1;                // начальные значения коэффициентов метода прогонки
	for (size_t i = 1; i < n; ++i) {
