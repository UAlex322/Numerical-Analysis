...
{
    QStandardItemModel *model = new QStandardItemModel(rows,9,this);
    ...
    model->setHeaderData(4, Qt::Horizontal, "v_i - v_2i");
    model->setHeaderData(5, Qt::Horizontal, "ОЛП");
    model->setHeaderData(6, Qt::Horizontal, "h_i");
...
'v0'    - начальная линейная скорость маятника [м/c].\n\n\
...
// Заполнение таблицы
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        model->setData(model->index(i,4), solution[i].v[0] - solution[i].v2[0]);
        model->setData(model->index(i,5), solution[i].s_star);
        model->setData(model->index(i,6), solution[i].h);
        model->setData(model->index(i,7), solution[i].c_plus);
        model->setData(model->index(i,8), solution[i].c_minus);
    }
...
// Создание справки и её отображение
    ...
    solutionInfoString += "\nε = " + QString::number(16.0/15.0*v_eps) + ", ε_min = " + QString::number(v_eps/30.0);
